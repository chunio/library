...
{//
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if(1){//button
            ...
            try{
                $jwtArray = explode('.', $auth);
                [$payloadBase64, $tokenBase64, $signatureBase64] = $jwtArray;
            }catch (\Throwable $e){
                throw new Exception("hacker attack ({$auth} is incorrect)");
            }
            //DEBUG_LABEL[START]
            if(in_array(env('APP_ENV'), ['local', 'sandox', 'test', 'official'/*TODO:待優化*/])){
            ...
            if(($currentTime - $payload['timestamp']) > config('system.middleware.signature.timeout',7200)) {
                $code = env('APP_ENV') == 'prod' ? 401 : ErrorCodeSys::AUTH_TIMEOUT;//DEBUG_LABEL
            ...
            //step2：參數鍵名降序排序
            ksort($tempStruct);
            //$serverSignature = strtolower(md5(http_build_query($tempStruct) . $secret));
            $serverSignature = strtolower(md5(urlencode(http_build_query($tempStruct)) . $secret));
            //$serverSignature = strtolower(md5(str_replace('\\', '', json_encode($tempStruct, JSON_UNESCAPED_SLASHES/*|JSON_UNESCAPED_UNICODE*/)) . $secret));
            //xdebug(json_encode($tempStruct, JSON_UNESCAPED_SLASHES/*|JSON_UNESCAPED_UNICODE*/),'json encode');
            //xdebug(str_replace('\\', '', json_encode($tempStruct, JSON_UNESCAPED_SLASHES/*|JSON_UNESCAPED_UNICODE*/)),'str_replace json encode');
            $alarm = [
                '$payload' => $payload,
                '$token' => $token,
                ...
                //'json_encode()' => str_replace('\\', '', json_encode($tempStruct, JSON_UNESCAPED_SLASHES/*|JSON_UNESCAPED_UNICODE*/)),
            ];
        ...
        Context::set('ryenovelPayload', $payload ?? []);
        //signature[END]
